"""Scene setup utilities for the Depal project."""
...
def create_scene_materials(
    ...
    components = create_materials_from_directory(
        ...
        base_material_usd_path=base_material_usd_path_str,
        material_config=material_cfg,
    )
    prim_paths = [
        comp.material_prim.GetPath().pathString
...
def setup_scene_floor(stage, floor_prim_path, all_pbr_components):
    ...
    if all_pbr_components:
        ...
        if not (
            floor_material_comp
            and floor_material_comp.material_prim
            and floor_material_comp.material_prim.IsValid()
        ):
    ...
    floor_component = (
        FloorMaterialComponent(
            material_prim=floor_material_comp.material_prim,
            shader_prims=floor_material_comp.shader_prims,
            material_schema=floor_material_comp.material_schema,
        )
...
def setup_scene_lighting(stage, light_cfg, light_base_path):
    ...
    lights = spawn_variable_random_lights(
        ...
        num_lights_config=tuple(light_cfg["num_lights_range"]),
        light_options=light_cfg["configurations"],
...
def spawn_main_asset(stage, asset_cfg, parent_path_str, base_pos_np, materials_folder_str, img_num):
    ...
    spawned_prim = spawn_single_pallet(
        ...
        prim_name=prim_name,
        position=base_pos_np,
        orientation_euler_degrees=(0.0, 0.0, rot_z),
        scale=scale,
        mass=details["mass_kg"],
        semantic_label=details["semantic_label"],
...
def spawn_additional_ycb_objects(stage, ycb_cfg):
    ...
    prims = spawn_objects(
        ...
        usd_asset_paths=ycb_cfg["asset_list"],
        parent_path=ycb_cfg["spawn_parent_path"],
        base_position=np.array(ycb_cfg["base_pos_xy"]),
        base_z_offset=ycb_cfg["spawn_z_offset"],
        z_jitter=ycb_cfg["spawn_z_jitter"],
        xy_jitter_range=tuple(ycb_cfg["spawn_xy_jitter_range"]),
        asset_scale_min=ycb_cfg["spawn_scale_min"],
        asset_scale_max=ycb_cfg["spawn_scale_max"],
...
def spawn_boxes_on_scene(stage, box_cfg, parent_path_str, base_pos_np, direct_mat_paths_list):
    ...
    spawned = spawn_basic_boxes(
        stage=stage,
        num_to_spawn_range=box_cfg["num_to_spawn_range"],
        parent_path=parent_path_str,
        base_position=boxes_base_pos,
        base_z_offset=0,
        z_jitter=box_cfg["z_jitter"],
        xy_jitter_range=tuple(box_cfg["xy_jitter_range"]),
        box_asset_paths=box_cfg["asset_usd_paths"],
        box_scale_min=box_cfg["scale_min"],
        box_scale_max=box_cfg["scale_max"],
        box_mass=box_cfg["mass_kg"],
        default_color=Gf.Vec3f(box_cfg["default_color_rgb"]),
